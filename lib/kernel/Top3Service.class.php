...
/**
 * Implement a Fia-Net's service (Certissim, Top3 or Sceau) *
 *
 *
 * 
 ...
 * @method void setCryptversion(string $status) sets the local var status
 ...
 * @method string getCryptversion(string $status) returns the local var status value
 * 
 ...
 * </code>
 */
abstract class Top3Service extends Top3Mother
{
	...
	private $urlscript = array(
		'eligibility' => array(
			'prod' => 'https://www.3foiscb-sofinco.fr/ws/v1/eligible/',
			'test' => 'https://rct.3foiscb-sofinco.fr/ws/v1/eligible/',
		),
		'frontline' => array(
			'prod' => 'https://www.3foiscb-sofinco.fr/front/xmlfeed',
			'test' => 'https://rct.3foiscb-sofinco.fr/front/xmlfeed',
		),
		'validatetransaction' => array(
			'prod' => 'https://www.3foiscb-sofinco.fr/ws/v1/validatetransaction/',
			'test' => 'https://rct.3foiscb-sofinco.fr/ws/v1/validatetransaction/',
		),
		'partiallyvalidatetransaction' => array(
			'prod' => 'https://www.3foiscb-sofinco.fr/ws/v1/partiallyvalidatetransaction/',
			'test' => 'https://rct.3foiscb-sofinco.fr/ws/v1/partiallyvalidatetransaction/',
		),
		'canceltransaction' => array(
			'prod' => 'https://www.3foiscb-sofinco.fr/ws/v1/canceltransaction/',
			'test' => 'https://rct.3foiscb-sofinco.fr/ws/v1/canceltransaction/',
		),
		'cancelprecedentorder' => array(
			'prod' => 'https://www.3foiscb-sofinco.fr/ws/v1/cancelprecedentorder/',
			'test' => 'https://rct.3foiscb-sofinco.fr/ws/v1/cancelprecedentorder/',
		),
		'gettransaction' => array(
			'prod' => 'https://www.3foiscb-sofinco.fr/ws/v1/gettransaction/',
			'test' => 'https://rct.3foiscb-sofinco.fr/ws/v1/gettransaction/',
		),
		'backoffice' => array(
			'prod' => 'https://www.3foiscb-sofinco.fr/bo/login',
			'test' => 'https://rct.3foiscb-sofinco.fr/bo/login',
		)
	);
	...
	private function loadParams()
	{
		foreach ($this->param_names as $param_name)
		{
			...
			if (version_compare(_PS_VERSION_, '1.5', '<'))
				$this->$funcname(Configuration::get('TOP3_'.Tools::strtoupper($param_name)));
			else
				$this->$funcname(Configuration::get('TOP3_'.Tools::strtoupper($param_name), null, null, $this->getIdshop()));
		}
	}
	...
	public function getUrl($script)
	{
		if (!array_key_exists($script, $this->url))
		{
			...
			Top3Logger::insertLogTop3(__METHOD__.':'.__LINE__, $msg);
			return false;
		}
	...
	public function switchMode($mode)
	{
		...
		{
			Top3Logger::insertLogTop3(__FILE__, "Le mode '$mode' n'est pas reconnu. 'test' défini à la place.");
			$mode = 'test';
		}
	...
	public function saveParamInFile()
	{
		foreach ($this->param_names as $param_name)
		{
			...
			if (version_compare(_PS_VERSION_, '1.5', '<'))
				Configuration::updateValue('TOP3_'.Tools::strtoupper($param_name), $this->$funcname());
			else
				Configuration::updateValue('TOP3_'.Tools::strtoupper($param_name), $this->$funcname(), false, null, $this->getIdshop());
		}
	}
